...
void scanString(char* string, int precision) {
  ...
  if (string != NULL) {
    if (precision == 0) {
      ...
      while ((precision-- > 0) && (!isspace(input) && (input != EOF))) {
        string[index++] = input;
        input = scanChar();
        found = TRUE;
        ++g_inChars;
...
long double scanLongDouble(void) {
  BOOL minus = FALSE, found = FALSE;
  long double longDoubleValue = 0.0L, factor = 1.0L;
  char input = scanChar();
  ...
  while (isdigit(input)) {
    longDoubleValue = (10.0L * longDoubleValue) + ((long double) (input - '0'));
    input = scanChar();
    found = TRUE;
  }
  ...
  if (input == '.') {
    ...
    while (isdigit(input)) {
      factor /= 10.0L;
      longDoubleValue += factor * ((long double) (input - '0'));
      input = scanChar();
      found = TRUE;
    }
  }
  ...
  if (tolower(input) == 'e') {
    ...
    longDoubleValue *= pow(10.0, exponent);
  }
  else {
    unscanChar(input);
  }
...
int scanFormat(char* format, va_list arg_list) {
  char c, *charPtr;
  BOOL percent = FALSE, shortInt, longIntOrDouble, longDouble, star = FALSE;
